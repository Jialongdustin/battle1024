defmodule Battle.Service.BattleService.ThreadPool do
  ...
  @appNames %{
    "battle-player-python" => "battle-player-lua",
    "battle-player-java" => "battle-player-c"
  }
  ...
  def handle_cast({:add_task, task}, state) do
    if length(state.workers) < state.size do
      ...
      {:noreply,
       %{state | workers: [worker | state.workers], busy: Map.put(state.busy, contest_id, worker)}}
    else
      # 否则，将任务加入队列
  ...
  def handle_info({contest_id, group_name, group_key, app_name}, state) do
    ...
    case :queue.out(state.queue) do
      {{:value, next_task}, new_queue} ->
        ...
        new_worker =
          Task.async(fn -> reuse_group_for_task(next_task, {group_name, group_key, app_name}) end)

        {:noreply,
         %{
           state
           | workers: [new_worker | workers],
             busy: Map.put(busy, next_contest_id, new_worker),
             queue: new_queue
         }}
  ...
  defp reuse_group_for_task(
         {user_id1, user_id2, contest_id, players},
         {groupName, groupKey, appName}
       ) do
