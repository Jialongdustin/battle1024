defmodule Battle.BattleDemo1 do
  require Logger

  def dfs_lady(chess,i,j,color,n,m,pre_i,pre_j) do
    best_paths = []
    best_paths =
      Enum.reduce(pairwise([-1, 0, 1, 0, -1]), best_paths, fn [qx, qy], acc ->
  ...
  def explore_capture(chess, i, j, qx, qy, lady, color, n, m, best_paths, nx, ny, rx, ry) do
    if rx >= 0 and rx < n and ry >= 0 and ry < m and Enum.at(Enum.at(chess, rx), ry) == 0 do
      ...
      current_paths = dfs_lady(new_chess, rx, ry, color, n, m, qx, qy)
      ...
      new_best_paths_2 = best_paths++new_best_paths
      ...
      # 累积所有路径
      best_paths_3 =
        Enum.reduce(further_paths, [], fn path, acc ->
          [path | acc]
        end)
      new_best_paths_2++best_paths_3
    else
      best_paths
    end
  end
        ...
        acc ->
          paths =
            case {white, Enum.at(Enum.at(turkish_flag, i), j)} do
              {true, 1} -> dfs(turkish_flag, i, j, false, "white", n, m)
              {true, 2} -> dfs_lady(turkish_flag, i, j, "white", n, m, 0, 0)
              {false, 3} -> dfs(turkish_flag, i, j, false, "black", n, m)
